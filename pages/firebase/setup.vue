  ...
  <ClientOnly>
    ...
    <div class="my-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
        <div v-for="(value, key) in configStatus" :key="key" class="p-2 rounded-lg"
          :class="value ? 'bg-green-50 dark:bg-green-900/30' : 'bg-red-50 dark:bg-red-900/30'">
          <div class="flex items-center justify-between">
            <span class="font-medium">{{ formatKey(key) }}</span>
            <i
              :class="value ? 'pi pi-check text-green-600/70 dark:text-green-400/70' : 'pi pi-times text-red-600/70 dark:text-red-400/70'"></i>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="mb-4">
      ...
      <div v-if="testResult"
        :class="['mt-2 p-2 rounded-lg', testResult.success ? 'bg-green-50 dark:bg-green-900/30' : 'bg-red-50 dark:bg-red-900/30']">
        <p class="font-medium text-sm"
          :class="testResult.success ? 'text-green-600/90 dark:text-green-400/90' : 'text-red-600/90 dark:text-red-400/90'">
          {{ testResult.message }}
        </p>
      </div>
    </div>
    ...
    <div>
      ...
      <div class="space-y-2">
        <div class="p-2 bg-blue-50 dark:bg-blue-900/30 rounded-lg">
          ...
          <p class="text-sm">Go to the <a href="https://console.firebase.google.com" target="_blank"
              class="text-blue-600/90 dark:text-blue-400/90 hover:underline">Firebase Console</a> and create a new
            project.</p>
        </div>
        <div class="p-2 bg-blue-50 dark:bg-blue-900/30 rounded-lg">
